...
wire csr_fault_w = is_csr_i && inst_valid &&( // CSR op is valid
    ...
    ((csr_wr_addr_i == `CSR_FFLAGS || csr_wr_addr_i == `CSR_FRM || csr_wr_addr_i == `CSR_FCSR) && mstatus_i[`SR_FS_R] == `SR_FS_OFF) || // read/write FPU CSRs when FPU off
...
always @(*) begin
    ...
    else if (mstatus_i[`SR_FS_R] == `SR_FS_OFF && is_f_ext_i)
        csr_exception_r = `EXCEPTION_ILLEGAL_INSTRUCTION;
    else if (is_fpu_done_i && mstatus_i[`SR_FS_R] != `SR_FS_OFF)
