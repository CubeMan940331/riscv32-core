...
//=================================
    ...
    input wire trap_ecall_i,
    input wire trap_ebreak_i,
    ...
    input wire csr_sel_i, // rs1 or imm
...
//=================================
    ...
    output wire trap_ecall_o,
    output wire trap_ebreak_o,
    ...
    output wire csr_sel_o, // rs1 or imm
    ...
    PipelineRegister #(.WIDTH(1))  reg_trap_ecall (.clk(clk), .rst_n(rst_n), .clear(clear), .en(en),  .data_i(trap_ecall_i), .data_o(trap_ecall_o));
    PipelineRegister #(.WIDTH(1))  reg_trap_ebreak (.clk(clk), .rst_n(rst_n), .clear(clear), .en(en),  .data_i(trap_ebreak_i), .data_o(trap_ebreak_o));
    PipelineRegister #(.WIDTH(1))  reg_inst_mret (.clk(clk), .rst_n(rst_n), .clear(clear), .en(en),  .data_i(inst_mret_i), .data_o(inst_mret_o));
    ...
    PipelineRegister #(.WIDTH(1))  reg_csr_wr_en (.clk(clk), .rst_n(rst_n), .clear(clear), .en(en),  .data_i(csr_wr_en_i), .data_o(csr_wr_en_o));
    PipelineRegister #(.WIDTH(1))  reg_csr_sel   (.clk(clk), .rst_n(rst_n), .clear(clear), .en(en),  .data_i(csr_sel_i), .data_o(csr_sel_o));
    // funct3 and funct7
